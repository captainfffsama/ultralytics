...
def feature_pca_v(
    ...
    """show the pca feature with image

    Args:
        img (_type_): _description_
        feature (_type_): _description_
        save_dir (_type_, optional): _description_. Defaults to Path('runs/detect/exp').
    """
    b, c, h, w = feature.shape
    feature = feature.view(b, c, -1).detach().permute(0, 2, 1)
    U, S, V = torch.pca_lowrank(feature, q=3)
    pca_f = torch.matmul(feature, V).permute(0, 2, 1).view(b, 3, h, w)
    pca_f = F.interpolate(pca_f, size=img.shape[-2:])
    pca_f = normalize_tensor(pca_f)
    img = normalize_tensor(img)
    final = normalize_tensor(img * 0.3 + pca_f * 0.7)

    alone_img = torch.chunk(final.cpu(), b, dim=0)
    for idx, i in enumerate(alone_img):
        file = save_dir / f"batch_{idx}_pca_{feature_name}.png"  # filename
        img_f = i.squeeze().permute(1, 2, 0)
        plt.imshow(img_f)
...
def detect_vis(
    img: torch.Tensor,
    detect_out: Tuple[torch.Tensor, List[torch.Tensor]],
    feature_name: str,
    reg_max: int,
    save_dir=Path("runs/detect/exp"),
):
    ...
    Args:
        img (_type_): _description_
        detect_out (_type_): _description_
        save_dir (_type_, optional): _description_. Defaults to Path('runs/detect/exp').
    """
    # model_out (B,4_nc,ch*cw)
    batchsize=img.shape[0]
    model_out, loss_out = detect_out

    # split cls hot map
    model_out_cls = model_out[:, 4:, :].detach()
    loss_out_cls = [x[:, 4 * reg_max :, :, :].detach() for x in loss_out]
    loss_out_shape = [x.shape[-2:] for x in loss_out]

    model_out_cls = torch.split(
        model_out_cls, [x[0] * x[1] for x in loss_out_shape], dim=2
    )

    model_out_cls = [
        x.view(x.shape[0], x.shape[1], y[0], y[1])
        for x, y in zip(model_out_cls, loss_out_shape)
    ]

    # a, _ = _split_channel2grid(loss_out[0][:, None, ...].detach().cpu(), 2)

    # synthe final vis image

    moc_syn_imgs=[_synthe_vis_img(moc, img) for moc in model_out_cls]
    for bi in range(batchsize):
        file = save_dir / f"batch_{bi}_moc_syn_{feature_name}.png"  # filename
        fig, ax = plt.subplots(
            1, 3, tight_layout=True
        )  # 8 rows x n/8 cols
        ax = ax.ravel()
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        for i,msi in enumerate(moc_syn_imgs):
            ax[i].imshow(msi[bi].squeeze())  # cmap='gray'
            ax[i].axis("off")
    ...
    loc_syn_imgs=[_synthe_vis_img(loc, img) for loc in loss_out_cls]
    for bi in range(batchsize):
        file = save_dir / f"batch_{bi}_loc_syn_{feature_name}.png"  # filename
        fig, ax = plt.subplots(
            1, 3, tight_layout=True
        )  # 8 rows x n/8 cols
        ax = ax.ravel()
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        for i,lsi in enumerate(loc_syn_imgs):
            ax[i].imshow(lsi[bi].squeeze())  # cmap='gray'
            ax[i].axis("off")
...
def normalize_tensor(data: Union[torch.Tensor, np.ndarray]):
    if isinstance(data, torch.Tensor):
        _range = data.detach().max() - data.detach().min()
        return ((data - data.min()) / _range * 255).int().cpu()
    else:
        _range = np.max(data) - np.min(data)
        return ((data - np.min(data)) / _range * 255).astype(np.uint8)
...
def _split_channel2grid(
    ...
    c, h, w = data.shape[chw_start_dim:]
    nrow = int(np.sqrt(c))
    n_t = c % nrow
    n_t = nrow - n_t if n_t else 0
    ncol = c // nrow + min(n_t, 1)

    if n_t:
        fill_t = torch.zeros((*data.shape[:chw_start_dim], n_t, h, w))
        data = torch.concatenate((data, fill_t), dim=chw_start_dim)

    c, h, w = data.shape[chw_start_dim:]

    data = torch.reshape(data, (*data.shape[:chw_start_dim], c // nrow, h * nrow, w))
    data = data.transpose(chw_start_dim, chw_start_dim + 1)  # BHCW
    data = data.reshape((*data.shape[:chw_start_dim], h * nrow, 1, -1))
    data = data.transpose(chw_start_dim + 1, chw_start_dim + 2)  # BHWC
...
def _synthe_vis_img(data: torch.Tensor, background: torch.Tensor,resize=True):
    ...
    background = background[:, :, None, ...].expand(
        data.shape[0], 3, data.shape[1], data.shape[2], data.shape[3]
    )

    background, _ = _split_channel2grid(background.detach().cpu(), 2)
    # BCHW
    background = background.squeeze(4)
    background=torch.permute(background, (0, 2, 3, 1)).numpy()
    background=normalize_tensor(background)

    # BHWC
    data, _ = _split_channel2grid(data.detach().cpu(), 1)

    data=data.detach().cpu().numpy()
    data=mpl.colors.Normalize()(data)
    magma = mpl.colormaps.get_cmap("viridis")
    data_magma = magma(data)[..., :3]
    data_magma = normalize_tensor(data_magma).squeeze(3)

    final=normalize_tensor(background*0.5+data_magma*0.5)
    return final
